...
### 6. `softprog-ws`
Exposes SOAP Web Services using Jakarta XML Web Services (JAX-WS) specifications.
- **Web Services:** `CustomerWS` (providing a paginated search operations for customer entities).
- **DTOs:** `CustomerSearchResultDTO`.

### 7. `softprog-rs`
Exposes RESTful Web Services using Jakarta RESTful Web Services (JAX-RS) specifications.
- **REST Resources:** `FilmsRS` (delivering film data payloads in JSON formats).
